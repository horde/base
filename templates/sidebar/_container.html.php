...
<div<?php if (!empty($container['id']) && !is_int($container['id'])) {
    echo ' id="' . $container['id'] . '"';
} if (isset($container['header']) && !empty($container['header']['collapsed'])) {
...
<?php echo $this->renderPartial('rowtree', ['collection' => $container['rows']]) ?>
<?php else: ?>
<div class="horde-resources">
<?php echo $this->renderPartial('rowresource', ['collection' => $container['rows']]) ?>
</div>
<?php endif ?>
<?php else: ?>
