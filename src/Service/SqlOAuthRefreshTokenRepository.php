...
class SqlOAuthRefreshTokenRepository implements RefreshTokenRepository
{
    public function __construct(
        private readonly Adapter $db,
        private readonly string $table = 'horde_oauth_refresh_tokens',
    ) {}
    ...
    public function persist(RefreshToken $token): void
    {
        $this->db->insert(
            ...
            . ' (token_id, access_token_id, client_id, identity_id, scope, expires_at, revoked)'
            . ' VALUES (?, ?, ?, ?, ?, ?, ?)',
            [
                $token->tokenId,
                $token->accessTokenId,
    ...
    {
        $this->db->update(
            'UPDATE ' . $this->table . ' SET revoked = 1 WHERE access_token_id = ?',
            [$accessTokenId]
        );
    }
