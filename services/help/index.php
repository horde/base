...
$sidebar_url = Horde::url($base_url->copy()->add([
    'show' => 'sidebar',
    'topic' => $topic,
]));

if ($show == 'index') {
    $main_url = Horde::url($base_url->copy()->add([
        'show' => 'entry',
        'topic' => $topic,
    ]));
...
switch ($show) {
    ...
    case 'sidebar':
        ...
        $tree->setOption(['target' => 'help_main']);
        ...
        switch ($vars->get('side_show', 'index')) {
                ...
                foreach ($topics as $id => $title) {
                    ...
                    foreach ($levels as $key => $name) {
                        ...
                        if (empty($added_nodes[$idx])) {
                            if ($key == $lcount) {
                                $node_params['url'] = $base_url->copy()->setRaw(true)->add([
                                    'show' => 'entry',
                                    'topic' => $id,
                                ]);
            ...
            case 'search':
                ...
                $searchForm->addVariable(_("Keyword"), 'keyword', 'text', false, false, null, [null, 20]);
