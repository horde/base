...
it in uncontrolled conditions. Deprecated, use closures instead.
...
---------------------
MD5 is the recommended algorithm to use for non-cryptographic hashing
actions. Although it has known collision issues, this is only important for
cryptographic security. For unique hashing purposes, it is plenty fast and has
good uniform distribution. MD5 produces 128-bit hash values.
